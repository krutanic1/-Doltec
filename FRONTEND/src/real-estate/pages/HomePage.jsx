...
const STATS = [
  { value: '25,000+', label: 'Verified Listings',  icon: '🏠' },
  { value: '10,000+', label: 'Happy Owners',        icon: '👥' },
  { value: '₹2.4Cr',  label: 'Avg. Deal Value',     icon: '💰' },
  { value: '99%',     label: 'Trust Score',          icon: '⭐' },
];

const QUICK = [
  { title: 'Saved Properties', desc: 'Revisit your shortlisted gems anytime.', to: '/real-estate/saved',         cta: 'Open Saved',   icon: '♡' },
  { title: 'Compare Listings', desc: 'Side-by-side comparison for smarter decisions.', to: '/real-estate/compare', cta: 'Compare Now', icon: '⇄' },
  { title: 'Post Property',    desc: 'List your property and attract verified buyers.', to: '/real-estate/post-property', cta: 'List Free', icon: '+' },
];

const WHY = [
  { title: 'RERA Verified', desc: 'Every listing passes multi-stage RERA and legal verification before going live.', icon: '✓' },
  { title: 'Market Intelligence', desc: 'Data-driven insights on locality pricing trends, infrastructure, and growth.', icon: '📊' },
  { title: 'Zero Brokerage', desc: 'Connect directly with verified owners — no middlemen, no hidden fees.', icon: '🤝' },
];
    ...
    <div style={{ background: '#fff', fontFamily: 'Inter,sans-serif', overflowX: 'hidden', width: '100%' }}>
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:0 }} className="re-stats-grid">
          {STATS.map((s, i) => (
            <div key={s.label} style={{
              ...
              <p style={{ fontSize:28, marginBottom:2 }}>{s.icon}</p>
      ...
      <section style={{ background:'#f9fafb', borderTop:'1px solid #e5e7eb', padding:'clamp(48px,8vw,90px) clamp(16px,4vw,28px)' }}>
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginBottom:44 }}>
            <div>
              <p style={{ fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:'.18em', color:'#d97706', margin:'0 0 12px' }}>Handpicked</p>
              <h2 style={{ fontSize:36, fontWeight:900, color:'#0a1628', margin:0, letterSpacing:'-.03em' }}>Featured Properties</h2>
              <p style={{ fontSize:15, color:'#6b7280', margin:'10px 0 0', fontWeight:400 }}>Exclusive projects curated for discerning buyers</p>
            </div>
